...
resource "google_service_account" "terraform" {
  ...
  description  = "Runs production Terraform from GitHub Actions."
...
resource "google_service_account" "prod_deploy" {
  ...
  description  = "Builds and deploys production Cloud Run from main."
...
resource "google_service_account" "runtime" {
  ...
  description  = "Runtime identity for Medlock Cloud Run services."
...
resource "google_service_account_iam_member" "terraform_wif_prod" {
  service_account_id = google_service_account.terraform.name
  role               = "roles/iam.workloadIdentityUser"
  member             = local.github_prod_principal
}

resource "google_service_account_iam_member" "terraform_wif_prod_token_creator" {
  service_account_id = google_service_account.terraform.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = local.github_prod_principal
}

resource "google_service_account_iam_member" "prod_deploy_wif_prod" {
  service_account_id = google_service_account.prod_deploy.name
  role               = "roles/iam.workloadIdentityUser"
  member             = local.github_prod_principal
}

resource "google_service_account_iam_member" "prod_deploy_wif_prod_token_creator" {
  service_account_id = google_service_account.prod_deploy.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = local.github_prod_principal
}
